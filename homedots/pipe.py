...
class Application(object):
    ...
    def read(self, prop):
        setting = subprocess.check_output(
            f"pw-metadata -n settings 0 clock.force-{prop}", shell=True
        )
    ...
    def change(self, value, prop):
        subprocess.run(
            f"pw-metadata -n settings 0 clock.force-{prop} {value}", shell=True
        )
        if prop == "rate":
...
class Window(tk.Tk):
    def __init__(self, file):
        ...
        self.message = tk.Label(
            master=self,
            text=f"Current sample rate: {file.read('rate')}",
            fg="#ECEFF4",
            bg="#2E3440",
        )
        ...
        self.message2 = tk.Label(
            master=self,
            text=f"Current buffer size: {file.read('quantum')}",
            fg="#ECEFF4",
            bg="#2E3440",
        )
